...
def is_external_asset(asset_path: str) -> bool:
    ...
    # Suporte para URLs sem esquema explícito (ex.: //cdn.exemplo.com/logo.png)
    return bool(parsed.netloc and not parsed.scheme)


def copy_asset(asset_path: Optional[str], output_dir: Path) -> Optional[str]:
    if not asset_path:
        return None
    ...
    if is_external_asset(asset_path):
        return asset_path

    src = (ROOT / asset_path).resolve()
    if not src.exists():
...
def copy_logo(logo_path: Optional[str], output_dir: Path) -> Optional[str]:
    return copy_asset(logo_path, output_dir)
...
def build_hero(
    ...
    *,
    ...
    if primary_cta.get("text") and primary_cta.get("link"):
        primary_link = str(primary_cta["link"])
        primary_target = "" if primary_link.startswith("#") else ' target="_blank" rel="noopener"'
        buttons.append(
            f'<a class="btn primary" href="{escape(primary_link)}"{primary_target}>'
            f"{escape(primary_cta['text'])}</a>"
        )
    if secondary_cta.get("text") and secondary_cta.get("link"):
        secondary_link = str(secondary_cta["link"])
        secondary_target = "" if secondary_link.startswith("#") else ' target="_blank" rel="noopener"'
        buttons.append(
            f'<a class="btn secondary" href="{escape(secondary_link)}"{secondary_target}>'
            f"{escape(secondary_cta['text'])}</a>"
        )

    button_html = "".join(buttons)

    image_alt = escape(data.get("image_alt") or site.get("name", ""))
    hero_media = (
        f'<div class="hero-media"><img src="{escape(image)}" alt="{image_alt}" loading="lazy"></div>'
        if image
        else ""
    )

    hero_class = "hero has-media" if image else "hero"

    return f"""
    <header class="{hero_class}" id="inicio">
...
def build_showcase(data: Dict[str, Any]) -> str:
    if not data:
        return ""

    title = escape(data.get("title", "Resultados"))
    description = escape(data.get("description", ""))
    items = data.get("items") or []
    ...
    for item in items:
        ...
        if not image:
            continue
        image_path = escape(str(image))
        item_title = escape(str(item.get("title", "")))
        caption = escape(str(item.get("caption", "")))
        image_alt = escape(str(item.get("image_alt", item.get("title", "Resultado de sorriso"))))
        cards.append(
            f"""
    ...
    return f"""
    <section id="resultados" class="section showcase">
        <div class="section-header">
            <span class="eyebrow">Portfólio</span>
            <h2>{title}</h2>
            {description_html}
        </div>
        <div class="showcase-grid">{cards_html}</div>
    </section>
    """
...
def build_cta(data: Dict[str, Any]) -> str:
    ...
    if button.get("text") and button.get("link"):
        button_link = str(button["link"])
        button_target = "" if button_link.startswith("#") else ' target="_blank" rel="noopener"'
        button_html = (
            f'<a class="btn accent" href="{escape(button_link)}"{button_target}>'
...
def generate_site(config_path: Path, *, output_dir: Path) -> Path:
    ...
    prepare_output_dir(site_output)

    logo_path = copy_logo(site.get("logo"), site_output)

    hero_config = config.get("hero") or {}
    hero_image_path = copy_asset(hero_config.get("image"), site_output)

    sections: "OrderedDict[str, str]" = OrderedDict()
    nav_sections: List[Dict[str, str]] = [{"id": "inicio", "label": "Início"}]

    hero = build_hero(hero_config, site=site, logo=logo_path, image=hero_image_path)
    sections["hero"] = hero
    ...
    showcase_config = config.get("showcase") or {}
    if showcase_config:
        processed_items = []
        for item in showcase_config.get("items") or []:
            image_path = copy_asset(item.get("image"), site_output)
            processed_items.append({**item, "image": image_path})
        showcase_html = build_showcase({**showcase_config, "items": processed_items})
    else:
        showcase_html = ""

    if showcase_html:
        sections["showcase"] = showcase_html
        nav_sections.append({"id": "resultados", "label": "Resultados"})
